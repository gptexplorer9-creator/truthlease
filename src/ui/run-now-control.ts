...
export interface RunNowStartResult {
  caseId: string;
  sessionId: string;
  turnId: string;
  turnStatus: string;
...
export async function loadRunNowStatus(fetchImpl: FetchLike = fetch): Promise<RunNowStatus> {
...
export async function startRunNow(caseId: string, fetchImpl: FetchLike = fetch): Promise<RunNowStartResult> {
  ...
  const value = await responseJson(response);
  for (const field of ["caseId", "sessionId", "turnId", "turnStatus", "approvalUrl", "startedAt"] as const) {
    if (typeof value[field] !== "string" || value[field].trim() === "") {
      throw new Error("Run Now response omitted " + field + ".");
    }
  }
  return value as unknown as RunNowStartResult;
}

function trustedApprovalTarget(value: string | undefined): string | undefined {
...
export function setupRunNowControl(): void {
  if (typeof document === "undefined") return;
  const panel = document.querySelector<HTMLElement>("#run-now-control");
  const button = document.querySelector<HTMLButtonElement>("#run-now-button");
  const statusTarget = document.querySelector<HTMLElement>("#run-now-status");
  const approvalLink = document.querySelector<HTMLAnchorElement>("#run-now-approval");
  if (!panel || !button || !statusTarget || !approvalLink) return;

  const render = (status: RunNowStatus): void => {
    ...
    if (target) approvalLink.href = target;
  };
