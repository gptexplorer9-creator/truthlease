...
function renderPriorState(model: CaseViewModel): string {
  ...
  return `<section class="stage stage--verified" id="verified" aria-labelledby="verified-title">
    ...
    <ul class="verification-list">
      ...
      ${exclusions.map((entry) => {
        const object = typeof entry === "object" && entry !== null && !Array.isArray(entry) ? (entry as JsonObject) : {};
...
export function renderCaseHtml(model: CaseViewModel): string {
...
export function renderLoadingHtml(caseId: string): string {
...
export function renderFeedErrorHtml(caseId: string, message: string): string {
