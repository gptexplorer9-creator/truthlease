  ...
  public async query<RowType extends SqlRow = SqlRow>(text: string, values: readonly unknown[] = []): Promise<SqlQueryResult<RowType>> {
    ...
    switch (tag) {
      ...
      case "ledger.event.insert": {
        const row: Row = { event_id: values[0], case_id: values[1], run_id: values[2], sequence: values[3], idempotency_key: values[4], request_sha256: values[5], connector_id: values[6], event_type: values[7], payload: values[8], payload_sha256: values[9], received_at: createdAt() };
...
describe("TruthLeaseLedger", () => {
  it("writes immutable case/run/event records and returns exact replays", async () => {
    ...
    const request = { eventId: "event-1", caseId: "case-1", runId: "run-1", sequence: 1, idempotencyKey: "event-1-key", connectorId: "bright_data", eventType: "evidence.received", payload: { text: "hostile content stays inert", tags: ["recall"] } } as const;
    ...
    expect(detail.events).toEqual([expect.objectContaining({ sequence: 1, payload: request.payload })]);
  });

  it("rejects a sequence gap, changed replay, and a forged authentication object", async () => {
    const ledger = await preparedLedger();
    const input = { eventId: "event-1", caseId: "case-1", runId: "run-1", sequence: 1, idempotencyKey: "event-1-key", connectorId: "bright_data", eventType: "evidence.received", payload: { value: 1 } };
