import { arrayValue, booleanValue, checkEvidenceContract, firstString, objectValue, } from "./case-model.js";
...
const STAGE_IDS = ["evidence", "proof", "approval", "patch", "verified"];
...
function defaultConnectionMessage(runtime) {
...
function normalizeRuntime(model, runtime, fallbackState) {
    const feedProvenance = runtime?.feedProvenance ?? model?.provenance ?? classifyFeedProvenance(model?.feedStatus);
...
function currentStageSummary(model) {
    const active = model.stages.find((stage) => stage.status === "active");
    if (active)
        return `${active.label} ${STATUS_LABELS[active.status].toLowerCase()}`;
    const caution = model.stages.find((stage) => stage.status === "failed" || stage.status === "denied" || stage.status === "stale");
    if (caution)
        return `${caution.label} ${STATUS_LABELS[caution.status].toLowerCase()}`;
...
function stageNote(model, key, title) {
...
function renderPriorState(model, runtime) {
...
function renderVerification(model) {
    ...
    <ul class="verification-list">
      ...
      ${exclusions.map((entry) => {
        const object = typeof entry === "object" && entry !== null && !Array.isArray(entry)
            ? entry
            : {};
...
function renderStageRail(model) {
    return `<nav class="stage-rail" aria-label="Case progress"><ol>${model.stages
        .map((stage, index) => `<li class="stage-rail__item stage-rail__item--${stage.status}"><a data-ui-key="stage-${stage.key}" href="#${stage.key}"${stage.status === "active" ? ` aria-current="step"` : ""}><span class="stage-rail__number">${String(index + 1).padStart(2, "0")}</span><span><strong>${stage.label}</strong><small>${STATUS_LABELS[stage.status]}</small></span></a></li>`)
        .join("")}</ol></nav>`;
}
...
function eventTone(event) {
    if (event.type.endsWith("failed") || (event.type === "evidence.fetched" && !checkEvidenceContract(event).valid))
        return "danger";
...
function renderRunActivity(model, runtime) {
    ...
    <ol class="activity-list">${model.events
        .map((event, index) => `<li class="activity-event activity-event--${eventTone(event)}">
...
function queueLink(caseId) {
    return `/?case=${encodeURIComponent(caseId)}`;
}
function queueItemCopy(entry) {
...
function renderCaseTable(runtime) {
    const caseTypes = [...new Set(runtime.queueCases.map((entry) => entry.caseType?.trim()).filter((value) => Boolean(value)))].sort();
    const count = runtime.queueCases.length;
    const countLabel = runtime.queueState === "loading"
        ? "Loading case index"
    ...
    const rows = runtime.queueCases
        ...
        .join("");
    const emptyRow = runtime.queueState === "loading"
...
function renderSystemSummary(runtime) {
...
export function renderCaseHtml(model, runtime) {
...
export function renderLoadingHtml(caseId, runtime) {
...
export function renderFeedErrorHtml(caseId, message, runtime) {
    const normalizedRuntime = normalizeRuntime(undefined, {
        ...runtime,
    ...
    const title = normalizedRuntime.terminalState === "offline"
